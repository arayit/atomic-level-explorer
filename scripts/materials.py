...
A fifth, cheap but decisive in the lab: does the ladder run on ONE colour?  Two colours means
two synchronized bursts.
...
def hop_grid(min_m=2, max_m=3, tol=0.150):
    ...
    g = sorted((m * HC / nm, nm, m) for nm in DRIVERS_NM for m in range(min_m, max_m + 1))
    bands, holes = [], []
...
def main() -> int:
    ...
    for r in rows:
        r["_single"] = len(set(r["colours_nm"].split("+"))) == 1
    ...
    L.append(f"Driver colours: {', '.join(str(n) for n in DRIVERS_NM)} nm. "
    ...
    L.append(f"- Only {len(singles)} ladders ({100 * len(singles) / max(1, len(rows)):.0f}%) run "
             "on ONE colour; the rest need two or more synchronized bursts. Among the "
             "single-colour ones the most productive driver is "
